...
$case = [Environment]::GetEnvironmentVariable('SYSTEM_CAPTURE_TEST_CASE')
...
function New-SyntheticPcapNg([string]$Path) {
    ...
    foreach ($timestamp in $(if ($arguments -contains '--drop-only' -or $case -eq 'no_target') { @() } else { @(1000000,1005000) })) {
...
if ($lowerProgram -eq 'pktmon.exe') {
    if ($arguments.Count -eq 0 -or $arguments[0] -eq 'help') {
        ...
        if ($case -eq 'add_fail' -and $state.filters.Count -eq 2) { exit 5 }
        $name = $arguments[2]
        ...
        if ($case -eq 'unknown' -and $state.filters.Count -eq 0) { Write-Output 'unrecognized response'; exit 0 }
        if ($case -eq 'foreign' -and $state.filters.Count -eq 0) { $state.filters = @([pscustomobject]@{name='Foreign';protocol='UDP';ip='10.0.0.1';port='99'}) }
        if ($case -eq 'cleanup_foreign' -and $Description -eq 'verify pktmon filters before cleanup') {
            $state.filters += [pscustomobject]@{name='Foreign';protocol='UDP';ip='10.0.0.1';port='99'}; Save-AdapterState
        }
        Write-Output '数据包筛选器:'
        if ($state.filters.Count -eq 0) { Write-Output '    无' }
        else {
            Write-Output '# 名称 协议 IP 地址 端口'
            $i=0
            foreach ($filter in $state.filters) {
                ++$i
                Write-Output "$i $($filter.name) $($filter.protocol) $($filter.ip) $($filter.port)"
            }
        }
    } elseif ($arguments[0] -eq 'filter' -and $arguments[1] -eq 'remove' -and $arguments.Count -eq 3 -and $arguments[2] -eq 'help') {
        Write-Output "pktmon 筛选器删除`n    删除所有数据包筛选器。"
    } elseif ($arguments[0] -eq 'filter' -and $arguments[1] -eq 'remove' -and $arguments.Count -ge 3) {
        throw 'named filter removal is unsupported on this fixture'
        $name = $arguments[2]
        ...
        if ([bool]$state.pktmonRunning) { Write-Output "收集的数据:`n    数据包计数器，数据包捕获`n记录程序参数:`n    记录程序名称: PktMon`n    日志模式: 内存缓冲区" } else { Write-Output '数据包监视器没有运行。' }
        ...
        if ($arguments -contains '--drop-only') { throw 'etl2txt --drop-only is unsupported' }
    ...
    if ($arguments -contains '-help' -or $arguments -contains '/?') {
        Write-Output 'WPR help: -start -stop -status -instancename'
        ...
        if ($case -eq 'wpr_fail') { Write-Error 'synthetic WPR startup failure' -ErrorAction Continue; exit 5 }
