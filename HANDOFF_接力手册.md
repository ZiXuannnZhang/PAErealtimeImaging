...
- `ring_svc_selftest.exe --data <14.dat> --svc <ImagingSvc.exe> --id 14 --grid-mm 0.1 --block 200
  ...
  - `--sys-delay-per-ch "d0w1,d0w2,d1w1,d1w2,...,d7w1,d7w2"`（16 个逗号分隔采样点）下发每通道双波长延时截断，空=各通道统一默认。
...
- **双波长延时截断**：每通道独立 `RingReconCudaConfig.sysDelay[8][2]`（[通道][波长]，
  1-based 采样点，默认 358/371；532nm=波长1、1064nm=波长2），JSON 下发格式为 8×2 嵌套数组；
  ImagingSvc 兼容旧版全局 [wl1,wl2] 并广播到所有通道。
