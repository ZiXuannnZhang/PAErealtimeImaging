...
public:
  ...
  */
  ...
  // 只读诊断：返回渐变关键字段原始值（不拷贝任何容器），排查内存被写坏
  struct GradientDiag {
    int levelCount = 0;
    int periodic = 0;
    int invalidated = 0;
    quintptr colorBufferD = 0;
    int colorBufferSize = -1;
    bool sane = false;
  };
  GradientDiag diagnostic() const;
...
public:
  ...
  // 批量填充全部单元格（data 为 value-major 布局，长度 keySize*valueSize），
  // 用于大尺寸环形重建帧的快速更新，避免逐像素 setCell 阻塞 UI。
  void setCells(const double *data);
...
public:
  ...
  QCPColorGradient::GradientDiag diagnosticGradient() const { return mGradient.diagnostic(); }
  QCPRange dataRange() const { return mDataRange; }
  QCPAxis::ScaleType dataScaleType() const { return mDataScaleType; }
