...
**分支性质**：DAS 双波长实时成像质量增强的 B 档实验链。按本决定停止维护与后续实现。

按 `HANDOFF.md` 的既定规则（`IGNORE_REMOTE_EXPERIMENT_BRANCH = TRUE`），
该分支的**源码、测试、文档与结论一律不写入本项目状态**。其结论只存在于该分支内部；
需要引用时单独检查该分支，不得当作 canonical `main` 的既有事实，也不得据此改写本文件其他章节。
