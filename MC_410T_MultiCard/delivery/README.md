...
- 工具链：Qt 6.8.0 (mingw_64) + MinGW 13.1.0 + Ninja + CMake 3.30，位于 `D:\Qt\Qt6.8.0`。
- ZeroMQ：已 vendor 到 `delivery/third_party/zeromq`（zmq.h/zmq.hpp + libzmq-v141 DLL + MinGW 导入库），CMake 默认使用该目录。
- Debug 一键构建：在 `delivery` 目录执行 `build_mingw_debug.cmd`（自动设置 PATH，执行 CMake configure + build + windeployqt）。
- VSCode 调试：F5 使用 `.vscode/launch.json` 的 MinGW 配置（自动先执行 build_delivery_debug 任务）。
- 产物：`build/mingw_debug/bin/MC410T_Receiver.exe` 与 `ImagingSvc.exe`。
- 注意：Qt 的 moc/uic/rcc 无法处理含中文的路径，工作区路径必须保持纯 ASCII（当前 `C:\Users\yyps\Documents\ChatGPT\realtime_imaging_migration` 满足要求）。