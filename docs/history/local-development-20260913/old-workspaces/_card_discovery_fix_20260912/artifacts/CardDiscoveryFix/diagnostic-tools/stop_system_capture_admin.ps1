...
try {
    ...
    # Flow completion is not evidence validation. ETW loss, clock alignment,
    # target protocol headers and WPR scheduling coverage are not yet proven.
    $analysisReady = $false
    ...
    $validationSummary = [ordered]@{
        ...
        analysisNotReadyReasons = @('clock_alignment_not_verified','etw_loss_unknown','target_protocol_header_not_verified','wpr_thread_coverage_not_verified')
