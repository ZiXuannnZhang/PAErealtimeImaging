...
void RingConfigDialog::buildUi()
{
    ...
    m_chkSplice->setToolTip("仅勾选配准模式时可用；勾选后各通道只反投影到各自扇区，\n"
                            "避免其他扇区通道在本扇区画出弧线伪影。");
    m_chkCfGate = new QCheckBox("CF模式");
    m_chkCfGate->setToolTip("相干因子门控(CF)：按像素跨通道相干性加权快照，\n"
                            "抑制失相干弧线伪影；与拼接模式互斥。");
    ...
    m_spnCfGain = new QDoubleSpinBox;
    m_spnCfGain->setRange(0.0, 1.0);
    m_spnCfGain->setSingleStep(0.05);
    m_spnCfGain->setDecimals(2);
    m_spnCfGain->setValue(0.50);
    m_spnCfGain->setToolTip("CF^gain 的增益指数：0.5=平方根（默认），0=不抑制，1=最强抑制。");
    fCh->addRow("CF增益指数", m_spnCfGain);
    m_cmbNonlinear = new QComboBox;
    m_cmbNonlinear->addItem("关闭", 0);
    m_cmbNonlinear->addItem("pCF", 1);
    m_cmbNonlinear->addItem("DMAS", 2);
    m_cmbNonlinear->setToolTip("非线性增强：pCF 压缩单通道强弧后相干求和；\n"
                               "DMAS 只保留多通道相干交点；与拼接模式互斥，可与 CF 叠加。");
    fCh->addRow("非线性增强", m_cmbNonlinear);
    ...
    for (int c = 0; c < 8; ++c) {
        ...
        fCh->addRow(QString("通道%1重建半径").arg(c + 1), m_spnRadiusCh[c]);
    }
    ...
    // 羽化宽度仅在拼接模式下可编辑。CF 模式与拼接模式互斥；CF 增益仅在 CF 勾选时可用。
    auto syncModes = [this]() {
        const bool reg = m_chkRegister->isChecked();
        ...
        const bool cfAvail = !m_chkSplice->isChecked();
        m_chkCfGate->setEnabled(cfAvail);
        if (!cfAvail) m_chkCfGate->setChecked(false);
        m_spnCfGain->setEnabled(cfAvail && m_chkCfGate->isChecked());
        const bool nlAvail = cfAvail;
        m_cmbNonlinear->setEnabled(nlAvail);
        if (!nlAvail) m_cmbNonlinear->setCurrentIndex(0);
    };
    for (int c = 0; c < 8; ++c)
    ...
    connect(m_chkRegister, &QCheckBox::toggled, this, syncModes);
    connect(m_chkSplice, &QCheckBox::toggled, this, syncModes);
    connect(m_chkCfGate, &QCheckBox::toggled, this, syncModes);
    syncRadiusEditable();
    syncModes();
...
void RingConfigDialog::restoreDefaults()
{
    ...
    const bool spliceDefault = regDefault && val("spliceMode", false).toBool();
    m_chkRegister->setChecked(regDefault);
    m_chkSplice->setChecked(spliceDefault);
    m_spnSpliceBlend->setValue(val("spliceBlendDeg", 1.5).toDouble());
    m_chkCfGate->setChecked(!spliceDefault && val("cfGate", false).toBool());
    m_spnCfGain->setValue(val("cfGain", 0.5).toDouble());
    {
        const int nlSaved = val("nonlinearMode", 0).toInt();
        const int nlMode = (nlSaved >= 0 && nlSaved <= 2) ? nlSaved : 0;
        m_cmbNonlinear->setCurrentIndex(
            spliceDefault ? 0 : m_cmbNonlinear->findData(nlMode));
    }
    for (int c = 0; c < 8; ++c)
    ...
    m_spnSysDelay1->setValue(val("sysDelay1", 358).toInt());
    m_spnSysDelay2->setValue(val("sysDelay2", 371).toInt());
...
void RingConfigDialog::saveDefaults()
{
    ...
    s.setValue("cfGate", m_chkCfGate->isChecked());
    s.setValue("cfGain", m_spnCfGain->value());
    s.setValue("nonlinearMode", m_cmbNonlinear->currentData().toInt());
    for (int c = 0; c < 8; ++c)
    ...
    s.setValue("sysDelay1", m_spnSysDelay1->value());
    s.setValue("sysDelay2", m_spnSysDelay2->value());
...
RingReconCudaConfig RingConfigDialog::config() const
{
    ...
    cfg.coherenceGate = (!cfg.spliceMode && m_chkCfGate->isChecked()) ? 1 : 0;
    cfg.coherenceGain = m_spnCfGain->value();
    cfg.coherenceEpsilon = 1e-12;
    cfg.nonlinearMode = cfg.spliceMode ? 0 : m_cmbNonlinear->currentData().toInt();
    for (int c = 0; c < 8; ++c)
