...
NetworkController::~NetworkController() {
    ...
    if (m_running) {
        ...
        for (auto& r : m_receivers) { if (!r->wait(500)) r->terminate(); }
        for (auto& p : m_processors) { if (!p->wait(500)) p->terminate(); }
        for (auto& s : m_savers) { if (!s->wait(500)) s->terminate(); }
        if (m_publisher) { m_publisher->wait(500); }
    }
    // 停止反馈监听线程
...
// ─────────────────────────────────────────────────────────────────────
void NetworkController::start(const AcqConfig& config, std::function<void()> onStarted, std::function<void()> onFailed) {
    if (m_running) stop();
    m_config  = config;
    m_running = true;
...
void NetworkController::stop() {
    ...
    m_stopThread = std::thread([this, recvMoved, procMoved, saveMoved, pubMoved, dispMoved]() {
        ...
        for (auto& r : *recvMoved) { if (!r->wait(WAIT_MS)) { r->terminate(); r->wait(200); } }
        recvMoved->clear();


        for (auto& p : *procMoved) { if (!p->wait(WAIT_MS)) { p->terminate(); p->wait(200); } }
        procMoved->clear();
        for (auto& s : *saveMoved) { if (!s->wait(WAIT_MS)) { s->terminate(); s->wait(200); } }
        saveMoved->clear();
        if (*pubMoved) { if (!(*pubMoved)->wait(WAIT_MS)) (*pubMoved)->terminate(); pubMoved->reset(); }
        dispMoved->clear();
