...
static QVector<double> makeSpectrumBins(int halfN, int origSamples,
                                        int dispSamples, double sampleIntervalNs) {
    QVector<double> bins(halfN);
    // fsOrigHz 由前端“采样率”控件决定：真实采集 250 MHz；回放验证数据（14.dat/11.dat）为 200 MHz
    const double fsOrigHz = (sampleIntervalNs > 0.0) ? 1e9 / sampleIntervalNs : 250e6;
    ...
    , m_grpImaging(nullptr)
{
    ...
    ui->spnSampleRate->setAccessibleName("采样率(MSa/s)");
    // 显示控制组
    ...
    // 环形重建图像：独立弹窗显示 + 随数据保存联动存 PNG
    connect(m_imagingController, &ImagingController::ringImageReady,
            this, [this](const QImage &wl1, const QImage &wl2, int seq) {
        saveReconImage(wl1, "532nm");
        saveReconImage(wl2, "1064nm");
        ++m_imagingFrameCount;   // 环形重建输出帧计数（反馈显示用）
        if (m_cmbImagingMode && m_cmbImagingMode->currentIndex() == 1) {
            ...
            const int nx = m_imagingController->ringFrameNx();
            m_imagingDisplayWindow->showRingImage(
                m_imagingController->latestRingFrame(0),
                m_imagingController->latestRingFrame(1),
                nx, nx, seq);
        }
    });
        ...
        if (status == "running") {
            ...
            if (m_lblImagingStatus && m_imagingEnabled) {
                ...
                if (m_imagingController && m_imagingController->isRingMode()) {
                    total = m_imagingController->ringConfig().alinesPerChannelPerFrame;
                    if (total <= 0) total = 1;
                    progress = (m_ringBlockCounter.load() % total) + 1;
                }
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        for (int card = 0; card < CARDS_PER_DISPLAY_GROUP; ++card) {
            for (int ch = 0; ch < 2; ++ch) {
                ...
                syncPlotViewport(m_plotsSpectrum[card][ch]);
            }
        }
...
void MainWindow::setupPlots()
{
    ...
    for (int card = 0; card < CARDS_PER_DISPLAY_GROUP; ++card) {
        for (int ch = 0; ch < 2; ++ch) {
            ...
            // 忽略内容 sizeHint：坐标轴范围/刻度变化不改变图窗尺寸，
            // 上下两图按固定 1:1 伸缩分配，避免重排导致图窗重叠
            freqPlot->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
            ...
            // 忽略内容 sizeHint：坐标轴范围/刻度变化不改变图窗尺寸，
            // 上下两图按固定 1:1 伸缩分配，避免重排导致图窗重叠
            specPlot->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
            specPlot->setMinimumSize(0, 0);
            specPlot->setOpenGl(false);
            specPlot->addGraph();
            specPlot->graph(0)->setPen(QPen(QColor(120, 220, 120), 2.0));
            specPlot->graph(0)->setAdaptiveSampling(false);
            specPlot->graph(0)->setAntialiased(true);
            specPlot->setNotAntialiasedElements(QCP::aeNone);
            specPlot->xAxis->setLabel("频率 (MHz)");
            specPlot->yAxis->setLabel("幅值");
            specPlot->xAxis->setLabelColor(QColor(180, 180, 180));
            specPlot->yAxis->setLabelColor(QColor(180, 180, 180));
            specPlot->xAxis->setTickLabelColor(QColor(160, 160, 160));
            specPlot->yAxis->setTickLabelColor(QColor(160, 160, 160));
            specPlot->xAxis->setBasePen(QPen(QColor(80, 80, 80)));
            specPlot->yAxis->setBasePen(QPen(QColor(80, 80, 80)));
            specPlot->xAxis->setTickPen(QPen(QColor(60, 60, 60)));
            specPlot->yAxis->setTickPen(QPen(QColor(60, 60, 60)));
            specPlot->xAxis->setSubTickPen(QPen(QColor(90, 90, 90)));
            specPlot->yAxis->setSubTickPen(QPen(QColor(90, 90, 90)));
            specPlot->xAxis->setRange(0, 1);
            specPlot->yAxis->setRange(0, 1);
            specPlot->setBackground(QBrush(QColor(30, 30, 30)));
            specPlot->axisRect()->setBackground(QBrush(QColor(38, 38, 38)));
            specPlot->xAxis->grid()->setVisible(true);
            specPlot->yAxis->grid()->setVisible(true);
            specPlot->xAxis->grid()->setPen(QPen(QColor(85, 85, 85), 0.5, Qt::SolidLine));
            specPlot->yAxis->grid()->setPen(QPen(QColor(85, 85, 85), 0.5, Qt::SolidLine));
            specPlot->xAxis->grid()->setSubGridVisible(true);
            specPlot->yAxis->grid()->setSubGridVisible(true);
            specPlot->xAxis->grid()->setSubGridPen(QPen(QColor(68, 68, 68), 0.5, Qt::DotLine));
            specPlot->yAxis->grid()->setSubGridPen(QPen(QColor(68, 68, 68), 0.5, Qt::DotLine));
            specPlot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom);
...
void MainWindow::createConnections()
{
    ...
    connect(ui->spnSampleRate, static_cast<void(QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),
            this, [this](double v) {
                if (v <= 0.0) return;
                m_sampleIntervalNs = 1000.0 / v;
                logMessage(QString("信号采样率已设置为 %1 MSa/s（%2 ns/点）；频谱显示立即生效，接收分包参数需重新下发配置")
                           .arg(v, 0, 'f', 1).arg(m_sampleIntervalNs, 0, 'f', 3));
                saveSettings();
            });
...
void MainWindow::onToggleSaveClicked()
{
    if (m_netController && m_netController->isSaving()) {
        ...
        // 实时重建图像随数据保存联动：PNG 保存到 保存目录/recon_png
        ...
        logMessage(QString("实时重建图像将同步保存为 PNG 至: %1").arg(m_reconSaveDir));
    }
}
    ...
    for (int slotIdx = 0; slotIdx < CARDS_PER_DISPLAY_GROUP; ++slotIdx) {
        ...
        if (!isTileView) {
            ...
            if (effMode == DM_FREQ && specPlot && specPlot->isVisible()) {
                ...
                QVector<double> mag = computeMagnitudeSpectrum(trace);
                if (!mag.isEmpty()) {
                    QVector<double> bins =
                        makeSpectrumBins(mag.size(), snap.sampleCount, nPts, m_sampleIntervalNs);
...
void MainWindow::loadSettings()
{
    ...
    // 采样率前端控件：真实采集 250 MSa/s；回放验证数据（14.dat/11.dat）为 200 MSa/s
    ui->spnSampleRate->blockSignals(true);
    ui->spnSampleRate->setValue(1000.0 / m_sampleIntervalNs);
    ui->spnSampleRate->blockSignals(false);
