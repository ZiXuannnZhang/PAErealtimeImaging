...
%   （C1 整改：判据走 padConvergenceCheck 统一入口——padRef 自比恒零禁止
%   作为候选，至少两个非参考 pad 通过且两两稳定、无再次超差；旧
%   convergedPad<=padRef 门即使其余 pad 全超差也通过，撤回）、p/p′/b 波形误差
%   （绝对+相对）、滤波组合与 wl2 覆盖、起端/终端敏感性用例。起终端敏感性另见
%   test_boundary_reference.m（纳入统一入口）。
...
% C1 整改：判据统一走 padConvergenceCheck（与 test_boundary_reference S3 同一
% 入口，标准不漂移）：padRef 只作参考（自比恒零禁止作为候选）、至少两个不同
% 非参考 pad 通过容差、两两稳定（实际两两差，同一参考尺度）、从首个通过 pad
% 到最大非参考 pad 不出现再次超差；候选不足/非有限/全超差一律 FAIL。旧
% "convergedPad<=padRef 即通过"即使其余 pad 全部超差也会通过，已撤回。
...
for sc6 = {'echoOnly', 'burstOnly', 'burstEcho'}
    ...
    nP6 = numel(padList);
    relP = zeros(1, nP6);  relPP = zeros(1, nP6);  relB = zeros(1, nP6);
    yPad6 = cell(1, nP6);  ppPad6 = cell(1, nP6);  bPad6 = cell(1, nP6);
    for ip6 = 1:nP6
        p6 = padList(ip6);
        ...
        bp = 2 * yp - 2 * tCut6 .* ppp;
        yPad6{ip6} = yp;  ppPad6{ip6} = ppp;  bPad6{ip6} = bp;
        relP(ip6) = max(abs(yp(w1c:w2c) - yR(w1c:w2c))) / sPc;
        relPP(ip6) = max(abs(ppp(w1c:w2c) - ppR(w1c:w2c))) / sPPc;
        relB(ip6) = max(abs(bp(w1c:w2c) - bR(w1c:w2c))) / sBc;
    end
    pairRel6 = struct('diffP', zeros(nP6), 'diffPP', zeros(nP6), 'diffB', zeros(nP6));
    for a6 = 1:nP6
        for b6i = 1:a6 - 1
            dP6 = max(abs(yPad6{a6}(w1c:w2c) - yPad6{b6i}(w1c:w2c))) / sPc;
            dPP6 = max(abs(ppPad6{a6}(w1c:w2c) - ppPad6{b6i}(w1c:w2c))) / sPPc;
            dB6 = max(abs(bPad6{a6}(w1c:w2c) - bPad6{b6i}(w1c:w2c))) / sBc;
            pairRel6.diffP(a6, b6i) = dP6;  pairRel6.diffP(b6i, a6) = dP6;
            pairRel6.diffPP(a6, b6i) = dPP6;  pairRel6.diffPP(b6i, a6) = dPP6;
            pairRel6.diffB(a6, b6i) = dB6;  pairRel6.diffB(b6i, a6) = dB6;
        end
    end
    [verdict6, det6] = padConvergenceCheck(padList, relP, relPP, relB, padRef, ...
        b6ref.convTolRel, pairRel6);
    b6ref.convergence.(rn) = struct('padList', padList, 'relP', relP, 'relPP', relPP, ...
        'relB', relB, 'scaleP', sPc, 'scalePP', sPPc, 'scaleB', sBc, ...
        'convergedPad', det6.firstOkPad, 'verdict', verdict6, 'detail', det6);
end
...
fprintf('pad 收敛判据（容差 %.0e，padRef=%d 自比不计入）：echoOnly=%s(firstOk=%g,nOk=%d) burstOnly=%s(firstOk=%g,nOk=%d) burstEcho=%s(firstOk=%g,nOk=%d)\n', ...
    b6ref.convTolRel, padRef, ...
    b6ref.convergence.echoOnly.verdict, b6ref.convergence.echoOnly.convergedPad, ...
    b6ref.convergence.echoOnly.detail.nOk, ...
    b6ref.convergence.burstOnly.verdict, b6ref.convergence.burstOnly.convergedPad, ...
    b6ref.convergence.burstOnly.detail.nOk, ...
    b6ref.convergence.burstEcho.verdict, b6ref.convergence.burstEcho.convergedPad, ...
    b6ref.convergence.burstEcho.detail.nOk);
...
res.policy = struct( ...
    ...
    'validQueryRegion', ['original-row index m >= max(sysDelay, dbrEnd+1), default margin G=0; when HP or LP zero-phase filtering is enabled ', ...
        '(with or without inversion), configs with E >= D are REJECTED with an error, where E = the ACTUAL DBR zeroed ', ...
        'length per channel/wavelength: E=0 when DBR is disabled or the effective zeroed length is 0 (a switch being on does not by itself imply a step), ', ...
        'E = min(maskLength+dbrmaskExtra, Nt) otherwise (default 313 < 358/371 satisfies; measured B2: a chopped startup burst leaves a zero-phase ', ...
        'response tail to ~tau 2737, exceeding dbrEnd+memHP=2327 and depending on the removed signal amplitude/length - no fixed margin can ', ...
        'guarantee validity). FROZEN C2 rule table (per enabled channel/wavelength; one violation rejects the whole config group with the channel ', ...
        'name and reason - see filterDbrConfigCheck.m / test_filter_dbr_config.m): filterEnabled=false -> no zero-phase-filter-specific rejection; ', ...
        'filterEnabled=true & E=0 -> allowed (sampling/cutoff/order/length checks still apply via refZeroPhase); filterEnabled=true & E>0 & ', ...
        'delayCut=true -> allowed only if E < D, error if E >= D ("DBR zeroed end must precede the delay-cut start for this channel/wavelength"); ', ...
        'filterEnabled=true & E>0 & delayCut=false -> REJECTED ("zero-phase filtering is incompatible with an uncut DBR-zeroed prefix; enable ', ...
        'delay-cut or disable DBR zeroing") - this combination is explicitly UNSUPPORTED by this version, no longer deferred to stage B. ', ...
        'DBR disabled: E interpreted as 0 (no zeroed region, no step; evidence: cut-line output bitwise identical for zeroRows=313 vs 0, gate G3). ', ...
        'Inversion-only (no HP/LP): no non-causal filter wrap occurs, so the E<D rejection is not required there; the valid-query rule m >= E+1 ', ...
        'still excludes the zeroed prefix. Short window: line length <= 3n rejected (A5). Out-of-range queries: maskOob zero (production ', ...
        'convention). Endpoint derivative: one-sided differences at line ends (timeDerivative.m); the cut-line one-sided vs uncut central-difference ', ...
        'difference at the first sample is quantified in exp3 T7 endpointProbe. NOTE: passing this config check is NOT a boundary-error-free or ', ...
        'losslessness guarantee; the quantified start/tail finite-window errors (B6.sensitivity) and real-hardware UNVERIFIED limits remain'], ...
    'headPolicy', ['no fixed head-margin discard and no preset quality threshold; boundary-convention error is quantified against a converged INDEPENDENT long-window ', ...
        'reference (B6.reference/rows: structural assertions, old self-comparison construction negative-controlled, pad convergence at recorded tolerance 1e-9 relative ', ...
        'via the shared padConvergenceCheck criterion - padRef self-comparison never counts as a passing candidate, at least two distinct non-reference pads must ', ...
        'pass, they must be mutually stable at the same reference scale, and no relapse beyond the first passing pad; p/p''/b waveform errors with absolute+relative scales). ', ...
        'Steady-start (pre-trigger-zero model) lines: near-boundary echoOnly differences at ~1e-9 relative or below; burst-contaminated starts and near-tail targets ', ...
        'show measurable finite-window effects (B6.sensitivity) - no exact-zero-error claim is made. Startup interference remains a data-dependent quantity ', ...
        'reported via burstOnly residue profile and burstEcho INT columns (B1/B6), not a fixed discard'], ...
    ...
    'twoStateQuery', ['unified notation (algorithm-stage-A.md section 2.2): D systemDelay (raw one-based acoustic zero), T physical travel time, ', ...
        's=fs*T, m raw one-based coordinate, q stored-array zero-based continuous query coordinate; cut line q_cut=s (m=q_cut+D); uncut calibrated ', ...
        'query q_raw=s+D-1 (m=q_raw+1=s+D); inversion time multiplier t=T=s/fs=(q_raw+1-D)/fs (NOT q_raw/fs). FROZEN C3 rule table (three ', ...
        'feature combos x two delayCut states; see test_query_policy.m): all-off -> q=s (cut) / q_legacy=s (uncut, old behavior); filter-only ', ...
        '(HP/LP, inversion off) -> q=s (cut) / q_legacy=s (uncut: filtering changes signal content only, never the query geometry; C2 config ', ...
        'precheck applies first); inversion on (+/- HP/LP) -> q_cut=s (cut, m=s+D) / q_raw=s+D-1 (uncut, m=q_raw+1=s+D; C2 config precheck ', ...
        'applies first). Only inversion triggers the calibrated uncut query - "compensate whenever inversion OR filtering is enabled" is ', ...
        'withdrawn as ambiguous; no stage-B alternative (e.g. forcing delayCut=true) is offered by this version. The legacy DAS uncut query ', ...
        'q_legacy=s (no compensation, "sample 1 = acoustic zero") is a separate historical compatibility behavior, documented separately and ', ...
        'never mixed into the proposed inversion table; the two-state derivative endpoint convention difference remains reported per exp3 T7 ', ...
        'endpointProbe, no bit-exact endpoint equivalence is claimed'], ...
...
% G1b 参考收敛（C1 整改）：三场景经 padConvergenceCheck 判 PASS（padRef 自比
%    恒零不作为候选；至少两个非参考 pad 通过；两两稳定；无再次超差）
convVerdicts = {b6ref.convergence.echoOnly.verdict, ...
    b6ref.convergence.burstOnly.verdict, b6ref.convergence.burstEcho.verdict};
gate.G1b_refConverged = all(strcmp(convVerdicts, 'PASS'));
