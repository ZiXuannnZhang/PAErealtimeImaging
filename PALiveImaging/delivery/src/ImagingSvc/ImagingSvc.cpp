...
void ImagingSvc::processRingConfigure(const QJsonObject &ring)
{
    ...
    cfg.rawColsPerBlock = ring["rawColsPerBlock"].toInt(cfg.rawColsPerBlock);
    cfg.wlOffset        = ring["wlOffset"].toInt(cfg.wlOffset);
    cfg.shiftWL2        = ring["shiftWL2"].toInt(cfg.shiftWL2);
    cfg.reconMode       = ring["reconMode"].toInt(cfg.reconMode);
    cfg.readMode        = ring["readMode"].toInt(cfg.readMode);
    cfg.daqHz           = ring["daqHz"].toDouble(cfg.daqHz);
    cfg.radius          = ring["radius"].toDouble(cfg.radius);
    cfg.alineRateHz     = ring["alineRateHz"].toDouble(cfg.alineRateHz);
    ...
    cfg.coverageDeg            = ring["coverageDeg"].toDouble(cfg.coverageDeg);
    cfg.theta0Deg              = ring["theta0Deg"].toDouble(cfg.theta0Deg);
    ...
    cfg.refCol = ring["refCol"].toInt(cfg.refCol);
    const QJsonArray cr = ring["corrows"].toArray();
    if (cr.size() >= 2) {
        cfg.corrows[0] = cr[0].toInt();
        cfg.corrows[1] = cr[1].toInt();
    }
    cfg.interpFactor = ring["interpFactor"].toDouble(cfg.interpFactor);
...
void ImagingSvc::processRingPulse()
{
    ...
    for (int c = 0; c < 8; ++c) {
        ...
        for (int j = 0; j < perCh; ++j) {
            ...
            pp.systemDelay = m_ringConfig.sysDelay[w];
            pp.dbrmaskExtra = (w == 1 ? m_ringConfig.sysDelay[1] - m_ringConfig.sysDelay[0] : 0);
    ...
    for (int w = 0; w < 2; ++w) {
        ...
        if (ring_recon_cuda_append_angles(m_ringCuda[w], flat[w].data(), nt, nd[w],
                                          wlAng[w].data()) != 0) {
            sendError(QString("ring_recon_cuda_append_angles failed: %1")
                          .arg(QString::fromUtf8(ring_recon_cuda_last_error())), 2009);
            return;
        }
    }

    // 读取归一化帧并写回
    std::vector<float> acc(static_cast<size_t>(m_ringFrameSize));
    std::vector<float> accW(static_cast<size_t>(m_ringFrameSize));
    QVector<float> wlFrames[2];
    for (int w = 0; w < 2; ++w) {
        ring_recon_cuda_get_state(m_ringCuda[w], acc.data(), accW.data(), nullptr);
        wlFrames[w].resize(m_ringFrameSize);
        for (int i = 0; i < m_ringFrameSize; ++i)
            wlFrames[w][i] = acc[i] / std::max(accW[i], 1e-12f);
    }
    ++m_ringBlockIndex;
    ...
    std::memcpy(fb, wlFrames[0].constData(),
                static_cast<size_t>(m_ringFrameSize) * sizeof(float));
    std::memcpy(fb + m_ringFrameSize, wlFrames[1].constData(),
                static_cast<size_t>(m_ringFrameSize) * sizeof(float));
    h->frame_seq++;
    h->frame_ready = 1;
    m_ringSharedMemory->unlock();
...
void ImagingSvc::sendRingFrameToHost()
{
