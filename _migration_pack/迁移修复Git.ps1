...
if (Test-Path -LiteralPath (Join-Path $WorkspaceRoot ".git")) {
    ...
    # 新 .git 必须由工作区用户创建，否则后续沙箱无法修改其保护 ACL。
    $executionAccount = [Security.Principal.WindowsIdentity]::GetCurrent().Name
    if ($executionAccount -match '\\CodexSandbox(?:Online|Offline)$') {
        throw 'Initialize Git from an approved non-sandbox command under the workspace owner account. Creating .git as a sandbox user can prevent later sandbox ACL setup.'
    }
