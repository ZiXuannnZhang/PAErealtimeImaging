...
void ImagingSvc::processRingConfigure(const QJsonObject &ring)
{
    ...
    cfg.coherenceGate   = ring["coherenceGate"].toInt(cfg.coherenceGate);
    cfg.coherenceGain   = ring["coherenceGain"].toDouble(cfg.coherenceGain);
    cfg.coherenceEpsilon = ring["coherenceEpsilon"].toDouble(cfg.coherenceEpsilon);
    cfg.nonlinearMode   = ring["nonlinearMode"].toInt(cfg.nonlinearMode);
    ...
    // CF 门控与拼接模式互斥；增益/epsilon 做服务端兜底 clamp
    if (cfg.spliceMode) cfg.coherenceGate = 0;
    cfg.coherenceGain = qBound(0.0, cfg.coherenceGain, 1.0);
    if (cfg.coherenceEpsilon <= 0.0) cfg.coherenceEpsilon = 1e-12;
    // 非线性增强：与拼接模式互斥；支持 0=关闭/1=pCF/2=DMAS
    if (cfg.spliceMode || cfg.nonlinearMode < 0 || cfg.nonlinearMode > 2)
        cfg.nonlinearMode = 0;
