        ...
        while (m_inputQueue.try_dequeue(pkt) && count++ < PROC_BATCH_SIZE) {
            ...
            if (m_hasFlushedOnce) {
                ...
                if (static_cast<int16_t>(pkt.triggerSeq - m_lastFlushedTriggerSeq) <= 0) {
                    ...
                    //       超过 expectedPackets×3 个连续丢弃后，强制重置锚点，
                    //       接受任意新触发序号，恢复正常处理。
                    ++m_consecutiveDiscards;
                    if (m_consecutiveDiscards >= m_expectedPackets * 3) {
...
// 
// computeFrequency
// 输入：group.freqA/B 存放 PacketAssemblyBuffer 解包后的 int16 原始值（cast 为 float）
// 输出：group.freqA/B 替换为瞬时频率（kHz）；group.phaseA/B_display 临时相位（rad，降采样前）
//
// 原理：
//   FPGA 在 250 MHz 时钟域完成差分相位计算，每个采样点的值为
//   Δφ_n = φ_n - φ_{n-1}，单位 int16 Q0.15（满量程 = π rad）
//   瞬时频率 f_n = Δφ_n / (2π  T_adc) = Δφ_int16 / 32768  (250MHz / 2π) / 1000 kHz
//             = Δφ_int16  FREQ_SCALE_KHZ
//   累积相位 φ_n = Σ Δφ_i / 32768  π（rad）
// 
void DataProcessor::computeFrequency(TriggerGroup& group) {
    const int n = group.sampleCount;
    if (n <= 0) return;

    // int32 Q16.16 定点数 → float 弧度 → 频率 kHz
    // 参考正确项目: scale = 1.0f / (1 << 16)
    // freq_kHz = int32 / 2^16 × 250e6 / (2π) / 1000
    const double adcFreqKhz = FPGA_ADC_FREQ_HZ / 1000.0;  // 250e6 → 250000 kHz
    const double maxIntVal  = (m_config.bitsPerChannel == 32)
        ? static_cast<double>(1LL << 16)   // Q16.16: 16位小数
        : static_cast<double>(1LL << 15);  // Q0.15: 15位小数
    const float scale = static_cast<float>(adcFreqKhz / (2.0 * M_PI) / maxIntVal);

    for (int i = 0; i < n; ++i) {
        float dphiA = group.freqA[i];
        float dphiB = group.freqB[i];
        group.freqA[i] = dphiA * scale;
        group.freqB[i] = dphiB * scale;
    }
}
