...
**dbrEnd<sysDelay 拒绝策略的适用边界（任务 §7.2 问项，algorithm-stage-A.md §3.3 同步）**：DBR 关闭 → dbrEnd 解释为 0（无置零段、无阶跃，检查平凡满足；证据：zeroRows=313 vs 0 裁剪线逐位一致，G3）；仅反演（无 HP/LP）→ 无零相位非因果回卷，不要求该拒绝，有效查询规则 m ≥ dbrEnd+1 仍排除置零前缀；delayCut=0 且 DBR 开启 → 置零前缀**构造上就在滤波输入内**（无裁剪），"阶跃在裁剪区外"的依据**不适用**于未裁剪线，滤波+delayCut=0+DBR 视为暂不支持，留待阶段 B 显式决策；短窗（线长≤3n）报错（A5）、越界查询 maskOob 置零、端点导数单侧差分（T7 探针量化）均维持。
