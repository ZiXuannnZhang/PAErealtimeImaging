...
void ImagingSvc::processRingConfigure(const QJsonObject &ring)
{
    ...
    // 双波长延时截断：每通道独立 [8][2]；兼容旧版全局 [wl1, wl2]（广播到所有通道）
    const QJsonArray sd = ring["sysDelay"].toArray();
    if (sd.size() >= 2) {
        const bool perChannel = (sd.size() >= 8) && sd[0].isArray();
        if (perChannel) {
            for (int c = 0; c < 8; ++c) {
                const QJsonArray sdCh = sd[c].toArray();
                if (sdCh.size() >= 2) {
                    cfg.sysDelay[c][0] = sdCh[0].toInt();
                    cfg.sysDelay[c][1] = sdCh[1].toInt();
                }
            }
        } else {
            const int d1 = sd[0].toInt();
            const int d2 = sd[1].toInt();
            for (int c = 0; c < 8; ++c) {
                cfg.sysDelay[c][0] = d1;
                cfg.sysDelay[c][1] = d2;
            }
...
void ImagingSvc::processRingPulse()
{
    ...
    for (int c = 0; c < 8; ++c) {
        ...
        for (int j = 0; j < perCh; ++j) {
            ...
            pp.systemDelay = m_ringConfig.sysDelay[phIdx][w];
            pp.dbrmaskExtra = (w == 1 ? m_ringConfig.sysDelay[phIdx][1] - m_ringConfig.sysDelay[phIdx][0] : 0);
            ...
            rads[w].push_back(static_cast<float>(
                m_ringConfig.radiusPerChannel[phIdx]));
