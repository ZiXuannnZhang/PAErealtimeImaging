...
## 诊断日志导出已集成（2026-09-06）

- 主窗口运行日志区域新增“导出诊断日志…”：选择本次/历史运行、现场说明与 ZIP 路径，后台导出。日志自动按运行保存，包含界面/Qt/启动日志、扫描依据、目标顺序、逐卡就绪来源、配置发送与反馈、重试及失败状态、只读网络快照、实际参数与程序身份。
- ZIP 包含 summary.txt、runtime.log、events.jsonl、network.json、settings.json、manifest.json；支持截点、SHA256 校验、异常历史恢复、写盘失败兜底与不完整提示。使用说明见 `MC_410T_MultiCard/delivery/docs/诊断日志导出.md`。
- 正式 Receiver 构建通过；独立日志、界面、网络快照和控制器测试通过。回环覆盖四卡成功、第五目标超时、18 字节重发与三次超时重试；后台导出与基线回放均为四卡各 80 次完整触发，无丢包/不完整触发。配置报文固定预期值校验通过。
- 未进行真实服务器扫描或物理卡验收，未确认“卡五失败”根因。自动虚拟发现仍保持删除，配置协议及成像算法未改变。当前源码改动尚未提交。
- 用户最新要求后续独立完成，不再使用子代理。

## 当前工作区状态覆盖说明（2026-09-05，优先于下方旧机记录）

### 真实采集优先：移除自动虚拟卡发现

- 正式监听程序仅扫描配置的真实网段；扫描无结果时停止启动并提示检查网络，不再自动探测本机虚拟采集卡。
- 保留 `--target-ips` 显式 IPv4 目标列表，拒绝非法、空或重复目标；单个 IP 对应一张卡，不再复制为四项。显式指定回环地址仍可用于历史联调，但没有自动模拟回退。
- 监听启动日志列出目标来源（扫描/显式参数）及完整卡号与 IP 对应关系。物理卡数与双通道数分别理解；18 字节上线、60 字节配置确认、逐卡重试和配置报文保持原实现。
- `PALiveImagingSimSender/` 和 `ogprog/` 保留作为历史验收工具/参考；本次不改模拟器行为或成像算法。
- 用户已验收同机模拟成像及配置反馈；“卡五失败”暂待下一次真实采集定位。额外可达 IP 被当成采集卡是初步假设，尚未确认第五项地址。本次清理不解决真实扫描的设备身份识别问题，不按猜测固定为 `192.168.0.2–5`。
- 清理后验证：当前 `build/mingw_make` 的 `MC410T_Receiver` 构建成功；提取实际参数解析函数的独立测试程序通过 13 项检查（含默认扫描、单卡、四卡、重复/非法输入和数量上限），新 EXE 对缺参、空值、非法地址、重复地址均在 UI 启动前以退出码 2 拒绝。12 个真实配置/扫描函数与修改前正文一致；空扫描直接返回的分支经静态检查确认，未进行真实网络扫描或采集验收。
- 构建日志：`MC_410T_MultiCard/delivery/build/mingw_make/remove_virtual_discovery_build.log`；参数测试产物与日志：同目录 `virtual_cleanup_qa/`。本次源码与文档改动尚未提交。

- 当前工作区：`D:\ChatGPT\PAERealtimeImaging`。
- 用户要求的修复前原始快照：`9342af13a9a7d7e632d0aba3d9e0d3726e275468`，分支 `master`，身份 `Codex <codex@local>`（仅仓库配置）。快照包含原有未提交源码、MATLAB、文档、截图、测试数据与迁移依赖；忽略的工具/旧构建产物及重复 bundle 不入库。
- 两个 backup 分支已恢复保留；`origin` 指向 `_migration_pack/realtime_imaging_migration.bundle`，是本地迁移归档，不是托管远程。
- 沙箱已修复：原 `.git` 属于 `CodexSandboxOnline`，导致设置只读保护 ACL 时失败。逐文件校验 166 个 Git 元数据文件后，以用户拥有的同内容目录替换；原目录保存在 `_tools/git-metadata-original_20260905_144324`。没有关闭沙箱或移除保护规则；日志已确认保护 ACL 应用成功、`errors=[]`。
- 普通沙箱命令、MinGW 完整构建，以及 selftest 的 QProcess 子进程 + SHM + ZMQ + CUDA 均已实际通过。本机当前不再需要默认使用 `--no-launch 1`；其他沙箱状态需按实测判断。
- 主构建目录仍为 `MC_410T_MultiCard/delivery/build/mingw_make`；模拟器为 `PALiveImagingSimSender/build`。两者已按新工作区路径重新配置并构建。
- 已重建 `MC410T_Receiver`、`ImagingSvc`、`ring_svc_selftest`、`ring_udp_replay` 和 `PALiveImagingSimSender`。CUDA DLL 未重编，与迁移包 SHA256 相同。
- 旧 CMake 目录保留为 `*_oldpath_20260905_144715`。旧 CUDA build 现为 `build/ring_recon_cuda_oldpath_20260905_144715`，仅作为历史证据；未来修改 CUDA 源码需另行配置/构建，不复用该旧缓存。
- 8 通道双波长两圈回归完成，`build/mingw_make/migration_selftest` 中 20 个 raw 与归档的 `selftest_out14_final_equal` 逐文件 SHA256 相同。
- 迁移及验证日志：`delivery/build/migration_20260905.log`、`delivery/build/migration_verify_20260905.log`；回归日志为 `build/mingw_make/migration_selftest.log`。
- 用户账户的注册表已核实为 `SampleIntervalNs="4.0"`、`DaqHz="2.5e+08"`；沙箱 HKCU 属于不同执行身份，缺键不代表用户配置丢失。
- 本次修复只修改迁移/验证脚本、模拟器 CMake、忽略规则及文档；算法源码未改。修复改动保留为未提交差异，快照提交不代表真实采集验收完成。
- 实施边界与待办不变：真实采集 v2 sysDelay 验收、长稳回归、RadiusCalibration 的角度符号核实及全孔径/合成真值验证仍待进行。

常用只读检查（沙箱跨账户时使用进程级精确目录信任；Git 写操作仍需按权限执行）：

```powershell
& .\_tools\git\cmd\git.exe -c safe.directory=D:/ChatGPT/PAERealtimeImaging --no-optional-locks status --short
pwsh -NoProfile -File .\_migration_pack\迁移后验证.ps1
```

下面为旧机交接记录，提交状态、旧绝对路径和沙箱故障结论以本节为准。
...
未跟踪（约定不擅自入库）：`RadiusCalibration/`、`testdata/`、`测试截图/`、`重建核心基准测试报告.md`、`重建核心基准测试_raw.csv` 等；提交前需用户确认。
