...
* `ctest --test-dir MC_410T_MultiCard/delivery/build/paimage_core --output-on-failure`：9/9 通过，包含 2 秒 UDP socket replay、trace、worker、protocol、conversion、LoopLog 检查；
...
无参数管理员入口现在会先从最新的应用 request 取得 `captureSessionToken`，再做输出目录归属校验；同一 request 的失败重试可以安全续做，真正不同 token 或 trial 的目录仍会被拒绝。
