...
Backend::Backend(Settings settings,std::vector<DataProcessor*> p,std::vector<FileSaver*> s,TraceWriter* trace)
    ...
    receiver_.feedbackSink=[this](int card,int type){if(feedbackSink)feedbackSink(card,type);};
}
...
bool Backend::startMeasurement(std::uint64_t session){
    return control_.start([&]{session_=session;receiver_.prepareStart(session);output_.beginSession(session);},
                          [&](bool ok){receiver_.completeStart(ok);});
}
