...
本目录代码主体已经完成，但它仍是**离线预校准工具**，不是已经完成正式全量标定的结果库。

截至当前已知状态：

- MATLAB 静态检查与基础数据读取流程已跑通；
- 角度对齐整数列循环互相关已做合成位移验证；
- 3 通道缩减搜索流程曾完成冒烟；
- 仍缺少明确确认的“每通道全孔径扫描”真实标定数据，因此不能把历史 testdata/01 的结果当作正式半径结论；
- 默认 8 通道完整 coarse+fine 搜索尚未在合适真实全孔径数据上形成正式项目验收结论。

推荐下一步：

1. 取得并确认真正的每通道全孔径标定数据；
2. 先用已知真值的合成数据验证角度位移符号与半径恢复；
3. 再用真实数据做 2~3 通道小规模检查；
4. 最后运行 8 通道完整搜索，并把输出半径送入 canonical main 的配准模式做端到端验证。

## 维护约束

- 不要对原始 Bscan 做 `circshift` 或其他数值改写；角度对齐通过角度向量实现；
- wl2 的相对列偏移同样通过角度/索引语义处理，不修改原始数据；
- 不要把普通多通道采集数据误当成每通道全孔径标定数据；
- 参考通道绝对半径误差会传递给其他通道，应选信噪比和聚焦曲线明确的参考通道；
- 若修改当前 CUDA/ImagingSvc 的几何或 preprocessing 语义，应重新核对本 MATLAB reference，而不是假设该目录自动同步；
- 本目录 README 是当前说明入口；旧 `HANDOFF.md` 的“未跟踪工作区/其他会话未提交改动”等会话状态已经过时，不再保留为当前事实。
