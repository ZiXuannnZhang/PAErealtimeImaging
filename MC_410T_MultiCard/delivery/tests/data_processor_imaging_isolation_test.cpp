...
int main(int argc,char** argv){QCoreApplication app(argc,argv);
 ...
 throwing.setDirectSaveSink([&](const TriggerGroupPtr&){++saved;return true;});auto e=std::make_shared<TriggerGroup>();e->cardId=0;e->isComplete=true;e->sampleCount=1;e->freqA={1};e->freqB={-1};e->quality.qualityUnknown=false;e->quality.assemblyComplete=true;e->quality.packetCoverageComplete=true;e->quality.packetLengthValid=true;e->quality.sampleLengthValid=true;auto er=throwing.deliverAssembled(e,true,true);
