...
typedef struct RingReconCudaConfig {
    ...
    // ---- 相干因子门控（CF-Gated DAS）----
    int    coherenceGate;       // 1 = snapshot 时用 CF^gain 加权（拼接模式下无效）
    double coherenceGain;       // CF 增益指数（0~1，默认 0.5）
    double coherenceEpsilon;    // CF 分母保护（>0，默认 1e-12）
    // ---- 非线性增强（signed DMAS / pCF，p 固定 0.5）----
    int    nonlinearMode;       // 0=关闭；1=pCF；2=signed DMAS（默认 0）
    int    shiftWL2;
...
// 扩展状态读取：按需返回 acc/accW/accV2/accP/accQ 与块数、本圈已累积 A-line 数。
// 任一输出指针可为 null；未启用的累积器对应输出填 0。
RING_RECON_CUDA_API int ring_recon_cuda_get_state_ex(
